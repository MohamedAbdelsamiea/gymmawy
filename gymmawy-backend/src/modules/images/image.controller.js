...
import { checkFileAccess } from "../../middlewares/fileAccessMiddleware.js";
import fs from "fs";
...
export const getImageAsDataUrl = async (req, res) => {
  try {
    ...
    // Check if this is a payment proof or private file
    if (decodedPath.includes('/payment-proofs/') || decodedPath.includes('/private/')) {
      ...
      } else if (decodedPath.includes('/private/')) {
        const privateFile = await prisma.upload.findFirst({
          where: {
            OR: [
              { url: { contains: fileName } },
              { url: { contains: decodedPath } }
            ]
          }
        });

        if (!privateFile || (privateFile.userId !== userId && req.user?.role !== 'ADMIN')) {
          return res.status(403).json({ error: { message: "Access denied" } });
        }
      }
    }
...
export const getImageInfo = async (req, res) => {
  try {
    ...
    if (decodedPath.startsWith('/payment-proofs/') || decodedPath.startsWith('/private/')) {
      const fileName = path.basename(decodedPath);
