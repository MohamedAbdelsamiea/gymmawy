...
export async function create(req, res, next) {
  try {
    const schema = z.object({
      ...
      maxRedemptionsPerUser: z.coerce.number().int().min(0).optional(),
...
export async function update(req, res, next) {
  try {
    ...
    if (data.maxRedemptions !== undefined) couponData.maxRedemptions = data.maxRedemptions;
    if (data.maxRedemptionsPerUser !== undefined) couponData.maxRedemptionsPerUser = data.maxRedemptionsPerUser;
