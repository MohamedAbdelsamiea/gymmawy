...
export async function getPricesByPurchasable(req, res, next) {
  try {
    const schema = z.object({
      ...
      purchasableType: z.enum(['PRODUCT', 'PLAN', 'PROGRAMME', 'MEDICAL', 'SUBSCRIPTION', 'ORDER', 'PROGRAMME_PURCHASE']),
...
export async function getPricesByCurrency(req, res, next) {
  try {
    const schema = z.object({
      ...
      purchasableType: z.enum(['PRODUCT', 'PLAN', 'PROGRAMME', 'MEDICAL', 'SUBSCRIPTION', 'ORDER', 'PROGRAMME_PURCHASE']).optional()
    });
