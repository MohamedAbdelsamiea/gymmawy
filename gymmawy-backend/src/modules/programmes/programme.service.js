...
import { createBilingualSearchClause } from "../../utils/bilingual.js";
import { generatePurchaseNumber } from "../../utils/identifiers.js";
...
export async function listProgrammes({ skip, take, q, sortBy, sortOrder, currency }) {
  const where = {
    AND: [
      { isActive: true },
      { deletedAt: null },
      q ? createBilingualSearchClause(q, 'name') : {},
    ],
  };
  ...
  // Fetch prices for all programmes if currency is specified
  if (currency) {
    const programmeIds = items.map(item => item.id);
    const prices = await prisma.programmePrice.findMany({
      where: {
    ...
    // Create a map of programmeId to price
    const priceMap = new Map();
    prices.forEach(price => {
      priceMap.set(price.programmeId, price);
    });

    // Add prices to programmes
...
export async function getProgrammes(query = {}) {
  ...
  // Create a map of programmeId to prices
  const priceMap = new Map();
  prices.forEach(price => {
    if (!priceMap.has(price.programmeId)) {
      priceMap.set(price.programmeId, []);
    }
    priceMap.get(price.programmeId).push(price);
  });
  ...
  const transformedProgrammes = programmes.map(programme => {
    ...
    // Get prices from the priceMap
    const programmePrices = priceMap.get(programme.id) || [];
    const priceEGP = programmePrices.find(p => p.currency === 'EGP')?.amount || 0;
    const priceSAR = programmePrices.find(p => p.currency === 'SAR')?.amount || 0;
    const priceAED = programmePrices.find(p => p.currency === 'AED')?.amount || 0;
    const priceUSD = programmePrices.find(p => p.currency === 'USD')?.amount || 0;
...
export async function purchaseProgramme(userId, programmeId, country = 'EG') {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const purchaseNumber = generatePurchaseNumber(purchase.id, purchase.purchasedAt);
...
export async function purchaseProgrammeWithPayment(userId, programmeId, paymentData) {
  try {
    ...
    // Get programme with prices from database
    ...
    // Get programme price for the requested currency
    const programmePrice = programme.prices.find(p => p.currency === currency);
    if (!programmePrice) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (validatedCoupon) {
      try {
        const { redeemCoupon } = await import('../coupons/coupon.service.js');
        await redeemCoupon(userId, validatedCoupon.code);
